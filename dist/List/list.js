        ...
        (function (list) {
            ...
            var ListColumnModel = /** @class */ (function () {
                function ListColumnModel(column, dc, maxVisibleFilterOptions, colDefIndex) {
                    ...
                    this.filterDateRangeEditMode = ko.observable();
                    ...
                    this.showCopyButton = column.showCopyButton;
                    this.columnTitle = column.columnTitle;
                    this.isColumnTitleLocalized = column.IsColumnTitleLocalized === true ? true : false;
                    ...
                    this.filterDisplayFromTo = ko.pureComputed(this.getFilterDisplayFromTo, this);
                }
                ...
                ListColumnModel.prototype.startFilterDateRangeEditMode = function (data, event) {
                    this.filterDateRangeEditMode(true);
                    $(event.target).next('input').trigger('focus');
                };
                ListColumnModel.prototype.closeFilterDateRangeEditMode = function (data, event) {
                    this.filterDateRangeEditMode(false);
                    $(event.target).closest('div').next().children('button').first().trigger('focus');
                };
                ListColumnModel.prototype.getFilterDisplayFromTo = function () {
                    var dtFrom = ko.unwrap(this.filterDisplayFrom);
                    var dtTo = ko.unwrap(this.filterDisplayTo);
                    if (!_.isEmpty(dtFrom) && !_.isEmpty(dtTo))
                        return dtFrom + " - " + dtTo;
                    if (!_.isEmpty(dtFrom))
                        return "> " + dtFrom;
                    if (!_.isEmpty(dtTo))
            ...
            var ListDataRecord = /** @class */ (function () {
                function ListDataRecord($dataStruct, columns, rowSelectedPropName, rowMarkedPropName, colorIndicatorPropName, checkboxInRowAttName) {
                    ...
                    _.forEach(columns, function (column) {
                        var displayValue = $dataStruct[column.dispName];
                        _this[column.dispName] = _this.getColumnValue(column, displayValue);
                        if (column.name !== column.dispName) {
                            if (!_this[column.name]) {
                                var underValue = $dataStruct[column.name];
                                _this[column.name] = _this.getColumnValue(column, underValue);
                            }
                        }
                    });
                }
                ListDataRecord.prototype.getColumnValue = function (column, attValue) {
                    var result;
                    var culture = window.sf.localization.currentCulture();
                    switch (column.dispDataType) {
                        ...
                        case 'integer':
                            if (attValue === null || _.isUndefined(attValue) || (typeof (attValue.$hasValue) === 'function' && attValue.$hasValue() === false)) {
                                ...
                                result = sffw.formatAsAmountOrCurrency(typeof (attValue.$asString) === 'function' ? attValue.$asString() : attValue, column.formatAsAmount, column.formatAsCurrency, column.minDecimalPlaces, column.currencySymbol);
                            }
                            else {
                                result = typeof (attValue.$asString) === 'function' ? attValue.$asString() : attValue;
                            }
                            break;
                        case 'decimal':
                            if (attValue === null || _.isUndefined(attValue) || (typeof (attValue.$hasValue) === 'function' && attValue.$hasValue() === false)) {
                                ...
                                result = sffw.formatAsAmountOrCurrency(typeof (attValue.$asString) === 'function' ? attValue.$asString() : culture.decimalToStr(attValue), column.formatAsAmount, column.formatAsCurrency, column.minDecimalPlaces, column.currencySymbol);
                            }
                            else {
                                result = typeof (attValue.$asString) === 'function' ? attValue.$asString() : culture.decimalToStr(attValue);
                            }
                            break;
                        case 'date':
                            if (attValue === null || _.isUndefined(attValue) || (typeof (attValue.$hasValue) === 'function' && attValue.$hasValue() === false)) {
                                ...
                                result = typeof (attValue.$asString) === 'function' ? attValue.$asString() : culture.dateToStr(moment(attValue).toDate());
                            }
                            break;
                        case 'bool':
                            if (attValue === null || _.isUndefined(attValue) || (typeof (attValue.$hasValue) === 'function' && attValue.$hasValue() === false)) {
                                ...
                                result = typeof (attValue.$asString) === 'function' ? attValue.$asString() : culture.boolToStr(attValue);
                            }
                            break;
                        default:
        ...
        (function (list) {
            var ListViewModel = /** @class */ (function () {
                function ListViewModel(params, componentInfo) {
                    ...
                    // flag to suspend notifications back to ListCtrl while applying filters that were just loaded from it
                    // it prevents making multiple identical odata requests after ListCtrl state was set by app script and setReady(true) was called
                    this.suspendFilterUpdates = false;
                    this.onMultipleDropdownChange = function (col, newValue) {
                        if (_.isArray(newValue)) {
                            if (!_this.suspendFilterUpdates) {
                    ...
                    this.copyToClipboard = function (record, column, event) {
                        var cellValue = record[column.name];
                        if (!navigator.clipboard) {
                            var $textarea = $('<textarea />').appendTo('body');
                            $textarea.text(cellValue);
                            $textarea.css('position', 'fixed'); // Prevent scrolling to bottom of page
                            $textarea.trigger('select');
                            try {
                                document.execCommand('copy'); // Security exception may be thrown by some browsers.
                                var message = _this.dataContext.$localize('List$$copyToClipboardSuccess');
                                if (_this.notificationsController) {
                                    _this.notificationsController.addSuccess({ message: message });
                                }
                            }
                            catch (ex) {
                                var message = _this.dataContext.$localize('List$$copyToClipboardErr');
                                if (_this.notificationsController) {
                                    _this.notificationsController.addWarning({ message: message });
                                }
                                else {
                    ...
                    this.iconClasses = {
                        iconSortAsc: params.iconSortAsc || 'fa fa-sort-alpha-asc',
                        iconSortDesc: params.iconSortDesc || 'fa fa-sort-alpha-desc',
                        iconClearFilter: params.iconClearFilter || 'fa fa-remove',
                        iconPageFirst: params.iconPageFirst || 'fa fa-fast-backward',
                        iconPagePrevious: params.iconPagePrevious || 'fa fa-play fa-flip-horizontal',
                        iconDataLoading: params.iconDataLoading || 'fa fa-refresh fa-spin',
                        iconDataLoadError: params.iconDataLoadError || 'fa fa-exclamation-triangle',
                        iconDataRefresh: params.iconDataRefresh || 'fa fa-refresh',
                        iconPageNext: params.iconPageNext || 'fa fa-play',
                        iconPageLast: params.iconPageLast || 'fa fa-fast-forward',
                        iconViewSettings: params.iconViewSettings || 'fa fa-cog',
                        iconCopy: params.iconCopy || 'fa fa-copy w3-small',
                        iconLeaveFilterEditMode: params.iconLeaveFilterEditMode || 'fa fa-compress'
                    };
                    this.notificationsController = params.notificationsController;
                    this.pageSizeValues = params.pageSizeValues || '5,10,20,30';
                    this.pageSizeValuesComputed = ko.pureComputed(function () {
                        var _a;
                        return _.map((_a = ko.unwrap(_this.pageSizeValues)) === null || _a === void 0 ? void 0 : _a.split(','), function (arrVal) {
                            return _.toNumber(arrVal);
                        });
                    });
                ...
                ListViewModel.prototype.setColumnFilters = function () {
                    var _this = this;
                    this.suspendFilterUpdates = true;
                    _.each(this.columns, function (col) {
                ...
                ListViewModel.prototype.onRowClick = function (row, column, event) {
                    ...
                    if (event) {
                        ...
                        $(event.target).closest('tr').trigger('focus');
                    }
                    else {
                        var idx = this.records().indexOf(row);
                        this.$element.find(".sffw-list table > tbody > tr:nth-child(" + (idx + 1) + ")").trigger('focus');
                    }
                    ...
                    switch (event.keyCode) {
                        case 40:
                            if (event.target) {
                                ...
                                $(event.target).next('tr').trigger('focus');
                                return false;
                            }
                        case 38:
                            if (event.target) {
                                ...
                                var i = this.records().indexOf(row);
                                this.setFocusedRow(i - 1);
                                $(event.target).prev('tr').trigger('focus');
                                return false;
                            }
                        default:
                            return true;
