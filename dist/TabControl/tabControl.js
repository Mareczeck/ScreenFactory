                    ...
                    switch (event.keyCode) {
                        ...
                        case 37: // šipka doleva
                            if (event.target) {
                                ...
                                var $liToSetFocus = $(event.target).parent().prevAll('li:visible').eq(0);
                                if ($liToSetFocus.length === 0) {
                                    $liToSetFocus = $(event.target).parent().siblings('li:visible').last();
                                }
                                var tab = $liToSetFocus.children('a');
                                tab.trigger('focus');
                        ...
                        case 39: // šipka doprava
                            if (event.target) {
                                event.preventDefault();
                                var $liToSetFocus = $(event.target).parent().nextAll('li:visible').eq(0);
                                if ($liToSetFocus.length === 0) {
                                    $liToSetFocus = $(event.target).parent().siblings('li:visible').first();
                                }
                                var tab = $liToSetFocus.children('a');
                                tab.trigger('focus');
                                if (this.activateAutomatically) {
